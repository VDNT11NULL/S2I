...
class InnerUNet(nn.Module):
    def __init__(self, in_channels, features): # (128, 32) but as per new H1 we want it to be (256, feat)  for now feat = 64
        #basically i/p here is [1,256,64,64]
        super(InnerUNet, self).__init__()
        self.down1 = Block(in_channels, features, down=True, act="leaky", use_dropout=False)      #d1 [1, 64, 32, 32]
        self.down2 = Block(features , features * 2, down=True, act="leaky", use_dropout=False)    #d2 [1, 128, 16, 16]
        self.down3 = Block(features * 2, features * 4, down=True, act="leaky", use_dropout=False) #d3 [1, 256, 8,  8]
        self.down4 = Block(features * 4, features * 8, down=True, act="leaky", use_dropout=False) #d4 [1, 512, 4,  4]

        self.bottleneck = nn.Sequential(                                                               #d5 [1, 512, 2,  2]
            nn.Conv2d(features * 8, features * 8, 4, 2, 1), nn.ReLU()
        )

        self.up1 = Block(features * 8, features * 4, down=False, act="relu", use_dropout=False)   #u1 [1, 256,  8,  8] 
        self.up2 = Block((features * 4) + (features * 4), features * 2, down=False, act="relu", use_dropout=False)   #u2 [1,  128, 16, 16]
        self.up3 = Block((features * 2)+ (features * 2), features, down=False, act="relu", use_dropout=False)       #u3 [1,  64, 32, 32]
        self.up4 = Block(features * 2 , features * 4, down=False, act="relu", use_dropout=False)          #u4 [1,  256, 64, 64]
...
class OuterUNet(nn.Module):
    def __init__(self, in_channels, features): # Assumming [bs, 3,512,512] input and features == 64
        ...
        self.inner_unet = InnerUNet(features * 4, features) # (256, 64)
        
        self.up1 = Block((features * 8)+(features * 8) , features * 8, down=False, act="relu", use_dropout=True)       #u1 [1, 512, 4, 4]
        self.up2 = Block((features * 8) + (features * 8), features * 8, down=False, act="relu", use_dropout=True)     #u2 [1, 512, 8 ,8] 
        self.up3 = Block(features * 8 * 2, features * 8, down=False, act="relu", use_dropout=True)    #u3 [1,  512,16 ,16]
        self.up4 = Block((features * 8) + (features * 2), features * 8, down=False, act="relu", use_dropout=False)   #u4 [1,  512, 32 ,32] 
        ...
        self.up7 = Block(features * 8, 3, down=False, act="relu", use_dropout=False)       #u7 [1,   256, 256 ,256]
